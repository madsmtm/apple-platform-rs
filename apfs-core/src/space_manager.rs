...
use crate::pod::{parse_le_i64, parse_le_u16, parse_le_u64};
...
impl SpaceManagerBlock {
    ...
    pub fn chunk_info_address_block_ids(
        ...
        for _ in 0..device.chunk_info_address_block_count {
            let oid = parse_le_i64(&mut offset, address.as_ref())?;
    ...
    pub fn internal_pool_bitmap_xid(&self) -> Result<TransactionIdentifierRaw> {
        let mut offset = self.internal_pool_bitmap_xid_offset as usize;
        let xid = parse_le_u64(&mut offset, self.bytes().as_ref())?;
    ...
    pub fn internal_pool_bitmap_offset(&self) -> Result<u64> {
        let mut offset = self.internal_pool_bitmap_offset as usize;

        parse_le_u64(&mut offset, self.bytes().as_ref())
    }
    ...
    pub fn internal_pool_bitmap_free_next_offsets(&self) -> Result<Vec<u16>> {
        ...
        for _ in 0..self.internal_pool_bitmap_block_count {
            res.push(parse_le_u16(&mut offset, self.bytes().as_ref())?);
        }
